...
def stringcheck(query,domain = 0):
    ...
    if domain != 0:
        if isIntorString(domain):
            ...
            values_websites = stringcheck_help(domain,query)
        else:
        ...
        for j in range( len(list_domain)):  
            values_websites = stringcheck_help(list_domain[j],query)
    return values_websites

def stringcheck_help(domain,query):
    try :
        values_websites = []
        i = 1
