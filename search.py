...
def Downloading_data(query,domain):
    a = 0
    global queries
    values_web = []
    while a != 'exit':
        if len(query) is 0:
            ...
            values_websites = validinputs(query,domain)
...
def key_word_domain_name(returned_list_,key_words):
    list_webpage =[]
    ...
    if domain != 0:
        if isIntorString(domain):
            ...
            if domain in dict_domain.keys():
                ...
                print( "\nEnter a valid Input")
                validinputs(query,domain)
    else:
        try:
            for j in range( len(list_domain)):  
                for result in search(query,tld = list_domain[j],num =50,stop=50,pause =2):
                    print (str(i) + "-->" + result)
                    i=i+1
...
def graph1(lst):
    ...
    histo_gram(li)
    return li    
def main(query,key_words,domain):
   returned_list,queries = Downloading_data(query,domain)
